...
Kotlin's default arguments can make APIs easier to understand as a user and easier to write as a developer. No more manually writing out function overloads with every possible useful combination of arguments. Furthermore, when reading these functions its easy to see what arguments can be left out and what values they use if you do.

That all sounds great, right? Most uses of Kotlin default arguments will work perfectly, bar any bad code you write but that isn't really Kotlin's fault. However in some use-cases, there is a flaw with how default arguments work. More precisely evolving the API of a platform without requiring existing code to recompile.
...
Where the caller of the function decides which arguments it provides values for and relying on the defaults for the rest. You can find a _little_ bit more information on this in the [Kotlin docs](https://kotlinlang.org/docs/functions.html#default-arguments).
...
- The `int` - Determines which arguments use their default values. This is done by executing a bitwise AND (`&`) against the `int` and a number that is increasing in powers of 2 (to represent binary numbers with a leading 1, e.g. 2 = `10` and 16 = `10000`). The `int` itself is determined by the call-site depending on which arguments are passed into the function. For example:
  ...
  |----------------------------|-----------------------------------|-------------------------|-------------------------------------|
  | <center>`25 & 1`</center>  | <center>`11001 & 00001`</center>  | <center>1</center>      | <center>✅</center>                 |
  | <center>`25 & 2`</center>  | <center>`11001 & 00010`</center>  | <center>0</center>      | <center>❌</center>                 |
  | <center>`25 & 4`</center>  | <center>`11001 & 00100`</center>  | <center>0</center>      | <center>❌</center>                 |
  | <center>`25 & 8`</center>  | <center>`11001 & 01000`</center>  | <center>1</center>      | <center>✅</center>                 |
  | <center>`25 & 16`</center> | <center>`11001 & 10000`</center>  | <center>1</center>      | <center>✅</center>                 |
...
Finally, as seen in the decompiled code, now that all variables are correctly set (`var1` to `var5`) they are passed into the _real_ function. Thus reaching the end of the process.
...
However, if you are using something that resembles a platform, that runs your application rather than the other way around, then it should still work without recompilation. A platform should be able to evolve without breaking everything using it, assuming that a major version update is not being made. This is where the inner workings of Kotlin's default arguments unravel.
...
|----------------------------------|----------------------------------|----------------------------|
| <center>`MyInterface`</center>   | <center>`MyInterface`</center>   | <center>✅</center>        |
| <center>`MyInterface.A`</center> | <center>`MyInterface.A`</center> | <center>✅</center>        |
| <center>`MyInterface.B`</center> | <center>`MyInterface.B`</center> | <center>✅</center>        |
| <center>`MyInterface.C`</center> | <center>`MyInterface.C`</center> | <center>✅</center>        |
| <center>`MyInterface.D`</center> | <center>`MyInterface.D`</center> | <center>✅</center>        |
| <center>`MyInterface.E`</center> | <center>`MyInterface.E`</center> | <center>✅</center>        |
| <center>`int`</center>           | <center>`MyInterface.F`</center> | <center>❌</center>        |
| <center>`Object` </center>       | <center>`int`</center>           | <center>❌</center>        |
| <center>-</center>               | <center>`Object`</center>        | <center>❌</center>        |
...
As mentioned previously, depending on the situation, recompiling the depending code might be the simplest solution. If that is a valid choice, I'd go with that. From now on though, we'll assume that it's not a good idea and consider how to keep everything working without recompilation.
...
The obvious downside is that you lose the benefit of default values all together, but you'll have to decide what works best for you and stick to it. Maybe a rule of "no default arguments" is easier to follow than "always add an overload" even though the outcome is completely the same (for that singular change).
...
That being said, with my little experience here, I am curious why the last two arguments on the generated synthetic function are put at the end rather than the front. That _small_ change _should_ allow functions with default arguments to become backwards compatible without recompilation. For example:
...
As I said, I do not know if there is specific reason it works this way or if it is just an oversight. If it's a mistake then maybe there is still hope that it can be corrected moving forward. It might even be noted but as a low priority, since the use-case of backwards compatibility without recompilation is quite specific and does not affect the majority of Kotlin developers. 
...
Kotlin's default arguments are extremely useful for writing accessible code without explicitly overload a function to express every possible useful combination of arguments. However, they are not perfect. They are not backwards compatible unless the code using an evolved function is also recompiled. For most scenarios, that is perfectly fine and I do not want to advocate that you remove all reliance on default arguments. Instead, I want to make sure that you have all the information you need to make the correct decision for your application, library, framework or platform.

My advice though, after looking into this feature, is to keep freely using default arguments for your application, library or framework code but if you are writing a platform that runs other developers' applications then carefully consider your use of this feature.